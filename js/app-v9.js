...
(function () {
  ...
  // Handoff flags — concept-reveal takes over #transition-headline from hero
  let hlOwnedByConceptReveal = false;
  let hlPastConceptReveal = false;
  ...
  function renderFrame(img) {
    ...
    if (isMobile) {
      // MOBILE: cover mode. Bias the horizontal crop toward the right so the
      // monkey (right side of the frame) sits inward, more fully in view,
      // leaving negative space on the left for the name.
      if (imgR > canvasR) {
        dh = H;
        dw = H * imgR;
        dx = (W - dw) * 0.78; // shift left → reveal more of the right side
        dy = 0;
      } else {
        dw = W;
        dh = W / imgR;
  ...
  function initScrollSystem() {
    ...
    ScrollTrigger.create({
      ...
      onUpdate(self) {
        ...
        if (!hlOwnedByConceptReveal && !hlPastConceptReveal) {
          const slideP = Math.max(0, Math.min(1, (p - 0.58) / 0.20));
          transitionHL.style.opacity   = Math.min(1, slideP * 1.4);
  ...
  function initWorks() {
    const panels = Array.from(document.querySelectorAll('.v8-panel'));
    const mq = window.matchMedia('(max-width: 767px)');

    // ── Reversible mobile DOM arrangement ──────────────────────────
    // Mobile: case study sits directly after its panel (inline, pushes
    // siblings down). Desktop: case studies live after .v8-panels as
    // siblings (original markup). We track the original anchor so we can
    // restore on resize — DevTools emulation toggles must not strand state.
    const panelsEl = document.querySelector('.v8-panels');
    const works    = panelsEl ? panelsEl.parentNode : null;
    let arranged   = null; // 'mobile' | 'desktop'

    function caseFor(panel) {
      return document.getElementById('project-' + panel.dataset.project);
    }

    function arrangeMobile() {
      if (arranged === 'mobile' || !panelsEl) return;
      panels.forEach((panel) => {
        const cs = caseFor(panel);
        if (cs) panel.insertAdjacentElement('afterend', cs);
      });
      arranged = 'mobile';
    }

    function arrangeDesktop() {
      if (arranged === 'desktop' || !works || !panelsEl) return;
      // Restore: append case studies back into the works section, in panel
      // order, immediately after .v8-panels.
      let anchor = panelsEl;
      panels.forEach((panel) => {
        const cs = caseFor(panel);
        if (cs) { anchor.insertAdjacentElement('afterend', cs); anchor = cs; }
      });
      arranged = 'desktop';
    }
    ...
    function applyLayout() {
      ...
      if (want === arranged) return;          // no breakpoint transition — skip
      if (want === 'mobile') {
        arrangeMobile();
      } else {
        // Transition into desktop: clear tap state + close any open studies.
        panels.forEach((p) => p.classList.remove('is-expanded'));
        document.querySelectorAll('.v8-case-study.is-open').forEach(closeProject);
        arrangeDesktop();
      }
      ScrollTrigger.refresh();
    }

    // ── Tap-to-expand (bound always; gated at click time by viewport) ──
    panels.forEach((panel) => {
      panel.addEventListener('click', (e) => {
        ...
        // Collapsing a panel also closes its open case study.
        panels.forEach((p) => {
          if (p.classList.contains('is-expanded')) {
            const cs = caseFor(p);
            if (cs && cs.classList.contains('is-open')) closeProject(cs);
          }
    ...
    document.querySelectorAll('.v8-panel__toggle').forEach((btn) => {
      btn.addEventListener('click', (e) => {
        ...
        if (isOpen) closeProject(caseStudy);
        else openProject(caseStudy);
      });
    });

    // Initial layout + react to breakpoint changes (resize / emulation).
    // matchMedia change is the primary signal; a debounced resize listener is
    // a fallback for environments that don't dispatch mq change reliably.
    applyLayout();
    mq.addEventListener('change', applyLayout);
    let rt;
    window.addEventListener('resize', () => {
      clearTimeout(rt);
      rt = setTimeout(applyLayout, 150);
    });
  }
  ...
  function openProject(caseStudy) {
    ...
    // Refresh ScrollTrigger after layout changes from the height tween.
    gsap.delayedCall(0.85, () => ScrollTrigger.refresh());

    // Desktop: snap to the case study. Mobile: keep the panel in frame so
    // the expansion reads as content pushing down, not a jump.
    if (window.innerWidth < 768) {
      if (panel) lenis.scrollTo(panel, { offset: -90, duration: 0.8 });
    } else {
  ...
  function initConceptReveal() {
    ...
    const tl = gsap.timeline({
      scrollTrigger: {
        ...
        scrub: 1,
        anticipatePin: 1,
        onEnter()     { hlOwnedByConceptReveal = true; hlPastConceptReveal = false; },
        onLeave()     { hlOwnedByConceptReveal = false; hlPastConceptReveal = true; gsap.set(transitionHL, { opacity: 0 }); },
        onEnterBack() { hlOwnedByConceptReveal = true; hlPastConceptReveal = false; },
        onLeaveBack() { hlOwnedByConceptReveal = false; hlPastConceptReveal = false; },
        onUpdate(self) {
  ...
  function init() {
    ...
    initProcess();
    initWorks();
    // Refresh after initWorks DOM rearrangement so brand practice ST positions are correct
